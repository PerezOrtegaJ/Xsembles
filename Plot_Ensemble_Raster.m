...
if new_figure
    if sort_vectors
        Set_Figure('Ensemblex (sorted)',[0 0 1200 700])
    else
        Set_Figure('Ensemblex',[0 0 1200 700])
    end
end
        ...
        if nnz(voltage.Stimuli)
            ...
            if nnz(stimuli)
                ...
                for i = 1:n_stims
                    ...
                    if i>4
                        Plot_Area(rescale(stim,0,max_y),0,Attenuate_Colors(Get_Color(i-4)),0.3)
                    else
                        Plot_Area(rescale(stim,0,max_y),0,Get_Color(i),0.3)
                    end
