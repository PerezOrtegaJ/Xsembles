...
%
%   Set_Label_Time(samples,sampleFrequency,sampleShift)
%
%       default: sampleShift = 0;
%
...
if(samples/sampleFrequency<=0.03)           % Less than 30 ms
    step = 0.005;
    xlabel('time (ms)');
    factor = 0.001;
elseif(samples/sampleFrequency<=0.1)        % Less than 100 ms
    step = 0.01;
    xlabel('time (ms)');
    factor = 0.001;
elseif(samples/sampleFrequency<=0.2)        % Less than 200 ms
    step = 0.025;
    xlabel('time (ms)');
    factor = 0.001;
elseif(samples/sampleFrequency<=1)          % Less than 1 s
    step = 0.1;
    xlabel('time (ms)');
    factor = 0.001;
elseif(samples/sampleFrequency<=3)          % Less than 3 s
    step = 0.5;
    xlabel('time (s)');
    factor = 1;
elseif(samples/sampleFrequency<=5)     % Less than 5 s
    step = 0.5;
    xlabel('time (s)');
    factor = 1;
elseif(samples/sampleFrequency<=15)     % Less than 15 s
    step = 1;
    xlabel('time (s)');
    factor = 1;
elseif(samples/sampleFrequency<=30)      % Less than 30 s
    ...
    xlabel('time (s)');
    factor = 1;
elseif(samples/sampleFrequency/60<2)    % Less than 2 min
    step = 20;
    xlabel('time (s)');
    factor = 1;
